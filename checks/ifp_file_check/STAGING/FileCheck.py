...
while True:
    if all([os.path.isfile(f) for f in filelist]) and glob.glob(os.path.join(informentDirectory)) or glob.glob(os.path.join('\\\\staging-roko-appserver\\InformentFiles\\SOURCE\\errors' )) :
